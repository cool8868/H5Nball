...
{
    public class ArenaBuffDataCore
    {
        #region Config
        const string MEMPrefixBuffMember = "ABFM_";
        const int MEMTickSecondsBuffMember = 1200;
        const int CNTPlayersOn = 7;
        #endregion

        #region Cache
        static IArenaBuffSync s_buffSync;
        static MemCacheClient s_memBuffMember;
        #endregion

        #region Singleton
        static readonly object s_lockObj = new object();
        static volatile ArenaBuffDataCore s_instnce = null;
        public readonly bool InitFlag = false;
        public static ArenaBuffDataCore Instance()
        {
            if (null == s_instnce || !s_instnce.InitFlag)
            {
                lock (s_lockObj)
                {
                    if (null == s_instnce || !s_instnce.InitFlag)
                    {
                        s_instnce = new ArenaBuffDataCore();
                    }
                }
            }
            return s_instnce;
