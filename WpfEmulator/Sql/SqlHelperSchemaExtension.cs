...
namespace Helpers
{
    public sealed partial class SqlHelper
    {
        ...
        public static string[] GetDbTableNames(string connectionString, string database)
        {
            string sql = "SELECT name from "+database+".sys.objects  where type='U' and name not in('sysdiagrams') order by name";
            DataTable dt = ExecuteDataTable(connectionString, sql);
            return dt.Rows.Cast<DataRow>().Select(row => row["name"].ToString()).ToArray();
        }

        public static string[] GetDbColumnNames(string connectionString, string table)
        {
            string sql = "select name from syscolumns where id=(select max(id) from sysobjects where xtype='u' and name='"+table+"')";
            DataTable dt = ExecuteDataTable(connectionString, sql);
            return dt.Rows.Cast<DataRow>().Select(row => row["name"].ToString()).ToArray();
        }
